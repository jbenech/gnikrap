...
[TIP]
====
For all XSensor values you will have a function `isStarted()` that will returns `true` if started or `false` if not. *If the XSensor
...
these methods, the script will be stopped.*
====


==== The xTouch XSensor

image::xsensor/xTouch_001_configure.png[xTouch - configuration mode, align="center"]
...
image::xsensor/xGyro_001_running.png[xGyro, align="center"]
