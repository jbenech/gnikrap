...
final public class SimpleEV3Screen implements EV3Device {
  ...
  public SimpleEV3Screen() {
    // textLCD = BrickFinder.getLocal().getTextLCD();
    lcd = BrickFinder.getLocal().getGraphicsLCD();
    setInverted(false);
  }
  ...
  /**
   * @param size available size: [S, M, L]. Default size is 'M'
   */
  public void setFont(char c) {
    ...
    // textLCD = BrickFinder.getLocal().getTextLCD(font);
    lcd.setFont(font);
  }
  ...
  public void drawText(int txt, int x, int y) {
    drawText(Integer.toString(txt), x, y);
  }

  public void drawText(char txt, int x, int y) {
    if (inverted) {
      // textLCD.drawString(Character.toString(txt), x, y, inverted);
      // lcd.drawChar(character, x, y, anchor)
    } else {
      // textLCD.drawChar(txt, x, y);
    }
  }

  public void drawText(String txt, int x, int y) {
    if (inverted) {
      // textLCD.drawString(txt, x, y, inverted);
    } else {
      // textLCD.drawString(txt, x, y);
    }
  }
}
