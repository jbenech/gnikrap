 ...
 * <p>
 * <h4>Security</h4> This is not a concern within Gnikrap, this a tool designed to make code injection tool :-)
 * </p>
 * <p>
 * <h4>HTTP server</h4> Undertow has been chosen because this is the only HTTP server with WebSocket support out of the box that I was able to run for more than a few minutes on the EV3 hardware with
 * good performances. I already run it for several hours without any impact on performance - ok with only one client, and a human-user activity (and not a test program injecting requests) but it's the
 * "standard" use-case. We have a LEGO brick not a server ;-).
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * <h4>REST vs WebSocket</h4> Initially I plan to write a "regular" REST API ... but ... I have serious performance issues. For our business case (a lot of call with very few data), http protocol add
 * too much of "useless" data, mainly in the http header (between 500 an 700 bytes both in the request and the answer). As a consequence, on the EV3 hardware I don't achieve to process http quick
 * enough (about 10 json-REST-http call by seconds can be processed). The solution to this problem is to move to a WebSocket API based on json messages. Here again, json has been preferred to XML
 * because it is lighter and quicker to process.
 * </p>
 * <p>
 * <h4>json parser</h4> JSon comparison result on the EV3 hardware (JRE 1.7):
 * 
 * <pre>
 ...
 * </p>
 * <p>
 * <h4>Threading</h4> Gnikrap is made of 7 threads (and the JVM threads, GC, etc.):
 * <ul>
 ...
 * </p>
 * <p>
 * <h4>External library dependencies</h4> A lot of libraries (spring, slf4j, etc.) exists and does useful things... but theses libraries are clearly not designed for an environment where the memory
 * and CPU are <i>very</i> limited. So we limit the dependencies to the minimum. While possible (eg. Logging, Javascript engine) the JDK classes are used, while not possible (like
 * ApplicationContext/Configuration/etc.) a basic implementation with the bare minimum needed by Gnikrap is implemented.
 * </p>
 */
public class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    // Init business stuff
    GnikrapApp gnikrap = new GnikrapApp(configuration);
    ...
    try {
      ...
      try {
        ...
        LOGGER.log(Level.SEVERE, "Can't stop server", ex2);
        System.exit(0); // Force exit as the clean stop don't work
      }
    }
  }
}
